![Hack The Box Calgary](images/hack-the-box-calgary-github-header-templates.png)
# 1-Welcome
Welcome to the first Meetup for Hack The Box: Calgary, CA!
...
3. Prizes (~5 minutes)
4. Socialization (Remaining time up to 8pm MST)
# Walkthrough
...
Make sure you create a [free Hack The Box account before](https://account.hackthebox.com/register) beginning this walk through.
...
2. Visit [this page]([Get Kali | Kali Linux](https://www.kali.org/get-kali/#kali-virtual-machines))to download a pre-built Kali Linux virtual machine. Make sure to pick the 64-bit Virtual Box edition. Follow [this video](https://youtu.be/vnX1NaF4K-Q?si=CaBKJJGIF3HaEePo)to install Kali Linux using Virtual Box.
...
After you have installed and are running the Kali Linux virtual machine (VM), visit https://www.hackthebox.com/ using your preferred web browser.

Navigate to HTB Labs. You may be asked to login after clicking the link below:
![Hack The Box Calgary](images/htb-labs.png)

After logging in, choose "Starting Point" within the left side navigation.
![Starting Point navigation](images/starting-point-navigation.png)

On the Starting Point landing page, you will see the machine named "Meow". This is the simplest machine on Hack The Box, and the perfect place to begin learning how to use Hack The Box Labs. You will see in the image below that the machine has been already Pwned by the currently logged in account. By the end of this walkthrough, you will have Pwned the "Meow" machine as well!
...
You will then be presented with the option to connect to the Starting Point server using either OpenVPN or Pwnbox. It is recommended that you choose OpenVPN if this is your first time using the platform. In the case that you are already a paid HTB VIP or VIP+ subscriber, choose Pwnbox. These subscriptions offer a generous or unlimited amount of the Pwnbox in-browser experience.
![Connect to Starting Point](images/connect-to-starting-point.png)
Leave the default "VPN Access" and "VPN Server" selections, and choose "TCP 443". Then click "DOWNLOAD VPN" You should see an .ovpn file within your downloads folder. This file will allow you to use the OpenVPN application to connect to the Hack The Box server.
...
`nmap` is a new command which allows us to activate the Nmap application.
...
But we aren't finished yet! The objective of Hack The Box is to find `flags` , which are files that hold strings of text that prove we exploited the vulnerabilities and gained access to a box.

Remember the `ls` command? Let's use this to view the files in the current working directory. You should see the following output:
![Flag](images/flag.png)
`flag.txt` appears to be the file we are looking for. We can use the `cat` command to view the contents of this file in the terminal.
![Flag](images/cat.png)
So where do we submit our flag?
![Flag](images/tasks.png)
Submit the flag text into the last task named "Submit root flag".

Once you have submitted the flag text, you will see a popup announcing that you have pwned the box. Congratulations, you just pwned the Meow box! But this is only the beginning of your ethical hacking journey. There is still so much to learn, so make sure to continue attending Hack The Box Calgary.

## Homework
Finish as many boxes in "Starting point" as you can before the October 2024 meetup.
Connect online with all other Hack The Box Calgary Meetup attendees.